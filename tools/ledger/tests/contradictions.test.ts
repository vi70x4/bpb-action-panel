...
describe("invariant: node-liveness-consistency", () => {
	it("no contradiction when latestByKey deduplicates — both ghost and sim report node.status for same node", () => {
		// NOTE: latestByKey keys by event.key, so two events with key "node.status"
		// will be deduplicated — only the latest (by logical_time) survives.
		// This means the node-liveness-consistency invariant cannot detect this
		// contradiction when both tools report under the same key.
		// The contradiction would need tool-qualified keys (e.g. node.status.ghost)
		// to be detectable, which the current schema does not define.
		const events = [
			makeTestEvent({
				key: "node.status",
				tool: "ghost",
		...
		const report = detectContradictions(events);
		// Sim event (logical_time=2) overwrites ghost in latestByKey,
		// so no contradiction is detectable with this schema.
		expect(report.hard_count).toBe(0);
	});
...
describe("invariant: keyspace-orphan-consistency", () => {
	it("DRIFT when clean state + orphan keys > 0", () => {
		...
		const c = report.contradictions.find(
