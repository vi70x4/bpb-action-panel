...
describe("normalizeKey", () => {
	...
	it("normalizes camelCase to dot-separated", () => {
		// NOTE: normalizeKey lowercases BEFORE applying the camelCase regex,
		// so pure camelCase like "dhtPeerCount" → "dhtpeercount" (no uppercase left to split).
		// This is a known limitation — use hyphens/underscores/dots for normalization.
		// The camelCase regex only triggers on un-lowercased input.
		// dht.PeerCount (already dot-separated, has uppercase) → camelCase regex runs first,
		// then lowercase: "dht.PeerCount" → regex → "dht.Peer.ount"? No, the regex
		// requires [a-z][A-Z] and lowercasing already happened.
		// In practice: normalizeKey is designed for hyphen/underscore/dot-delimited keys.
		// CamelCase support is limited to the regex match on the ORIGINAL input before toLowerCase.
		expect(() => normalizeKey("dhtPeerCount")).toThrow(
	...
	it("accepts qualified sub-keys under known prefixes", () => {
		...
		expect(normalizeKey("dht.peer.count.bootstrap")).toBe(
	...
	it("accepts non-canonical sub-keys under known domain prefixes", () => {
		...
		expect(normalizeKey("dht.peer.count.custom_tool")).toBe(
			"dht.peer.count.custom.tool",
		);
		expect(normalizeKey("keyspace.vless.count.extra")).toBe(
			"keyspace.vless.count.extra",
		);
	});

	// Edge cases
	it("throws for completely unknown keys", () => {
		expect(() => normalizeKey("random.garbage")).toThrow(
			/not in the canonical registry/,
		);
		expect(() => normalizeKey("foo")).toThrow(/not in the canonical registry/);
		expect(() => normalizeKey("x.y.z")).toThrow(
			/not in the canonical registry/,
		);
	});
	...
	it("normalizes mixed separators correctly", () => {
		...
		expect(normalizeKey("tunnel-status_provider")).toBe(
			"tunnel.status.provider",
		);
	});
...
describe("isValidKey", () => {
	...
	it("returns true for normalizable variants", () => {
		...
		// Pure camelCase does NOT normalize because toLowerCase runs before the
		// camelCase regex — no uppercase chars left to split on.
		expect(isValidKey("dhtPeerCount")).toBe(false);
	});
