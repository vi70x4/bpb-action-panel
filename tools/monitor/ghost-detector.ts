...
export type PeerStatus = "alive" | "ghost" | "stale" | "unreachable";
...
export function classifyPeer(peer: PeerRecord, now: Date): ClassifyResult {
	...
	if (peer.tombstoned) {
		...
		return {
			peer,
			status: "stale",
			detail: `tombstoned but record persists (expired ${minsAgo}m ago)`,
		};
	}
	...
	return {
		peer,
		status: "ghost",
		detail: `expired ${minsAgo}m ago, NO TOMBSTONE → GHOST`,
	};
}
	...
	try {
		...
		for (const protocol of PROTOCOLS) {
			...
			for (const peer of knownPeers) {
				...
				try {
					const rawBytes = await node.services.dht.get(
						new TextEncoder().encode(key),
					);
					if (!rawBytes) continue;

					const record: RawDHTRecord = JSON.parse(
					...
					if (!tombstoneCache.has(pid)) {
						try {
							const tb = await node.services.dht.get(
								new TextEncoder().encode(tombstoneKey),
							);
							tombstoned = tb != null;
						} catch {
							tombstoned = false;
					...
					if (doProbe && result.status !== "stale") {
						...
						if (!reachable && result.status === "alive") {
							result = {
								peer: peerRec,
								status: "unreachable",
								detail: `TCP connect failed to ${peerRec.host}:${peerRec.port}`,
							};
						}
					}
		...
		if (doProbe && result.status !== "stale") {
			...
			if (!reachable && result.status === "alive") {
				result = {
					peer,
					status: "unreachable",
					detail: `TCP connect failed to ${peer.host}:${peer.port}`,
				};
			}
		}
